...
import { ShieldAlert, Check, X, Search, Filter, FileJson, Database, Trash2, Loader2, Headphones, ExternalLink, CheckCircle } from 'lucide-react';
...
const AdminPanel: React.FC<AdminPanelProps> = ({ currentUser, onBack, onRecipeSelect, onTagClick }) => {
  ...
  // Security check: If a moderator tries to access admin tabs, reset to photos
  useEffect(() => {
      if (isModerator && ['users', 'database', 'import'].includes(activeTab)) {
          setActiveTab('photos');
      }
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-8 gap-4">
             <div className="flex items-center gap-3">
                 ...
                 <div>
                     ...
                     <p className="text-sm sm:text-base text-gray-500 dark:text-gray-400">{currentUser.name}</p>
                 </div>
             </div>
             <button onClick={onBack} className="w-full sm:w-auto px-4 py-2 rounded-lg bg-gray-200 dark:bg-gray-800 hover:bg-gray-300 dark:hover:bg-gray-700 text-gray-800 dark:text-gray-200 transition-colors">Выйти</button>
        </div>

        {/* Navigation Tabs - STRICT ROLE BASED */}
        <div className="flex border-b border-gray-200 dark:border-gray-700 mb-8 overflow-x-auto scrollbar-hide">
            <button 
                onClick={() => setActiveTab('photos')} 
                className={`px-4 sm:px-6 py-3 font-medium border-b-2 transition-colors whitespace-nowrap ${activeTab === 'photos' ? 'border-emerald-600 text-emerald-600' : 'border-transparent text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200'}`}
            >
                Модерация Фото
            </button>
            <button 
                onClick={() => setActiveTab('reports')} 
                className={`px-4 sm:px-6 py-3 font-medium border-b-2 transition-colors whitespace-nowrap flex items-center gap-2 ${activeTab === 'reports' ? 'border-emerald-600 text-emerald-600' : 'border-transparent text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200'}`}
            >
            ...
            {isAdmin && (
                ...
                <button 
                    ...
                    className={`px-4 sm:px-6 py-3 font-medium border-b-2 transition-colors whitespace-nowrap ${activeTab === 'users' ? 'border-emerald-600 text-emerald-600' : 'border-transparent text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200'}`}
                >
                    Пользователи ({users.length})
                </button>
                <button 
                    onClick={() => setActiveTab('database')} 
                    className={`px-4 sm:px-6 py-3 font-medium border-b-2 transition-colors whitespace-nowrap ${activeTab === 'database' ? 'border-emerald-600 text-emerald-600' : 'border-transparent text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200'}`}
                >
                    Управление БД
                </button>
                <button 
                    onClick={() => setActiveTab('import')} 
                    className={`px-4 sm:px-6 py-3 font-medium border-b-2 transition-colors whitespace-nowrap ${activeTab === 'import' ? 'border-emerald-600 text-emerald-600' : 'border-transparent text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200'}`}
                >
                    Импорт
                </button>
                </>
        ...
        {activeTab === 'photos' && (
            <>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {filteredPhotos.length === 0 ? <div className="col-span-full py-20 text-center text-gray-400">Нет фото в этой категории</div> : filteredPhotos.map((photo, idx) => (
