    ...
    <div className="max-w-6xl mx-auto px-4 pb-20 animate-fade-in">
      ...
      <div className={`grid grid-cols-1 ${!isReadOnly ? 'md:grid-cols-3' : 'md:grid-cols-1'} gap-6 md:gap-8`}>
        <div className={`md:col-span-1 ${isReadOnly ? 'max-w-2xl mx-auto w-full' : ''}`}>
           <div className="glass-panel rounded-3xl p-5 sm:p-6 sticky top-24 text-center relative dark:bg-gray-800/50">
              {!isReadOnly && <button onClick={() => setIsSettingsOpen(true)} className="absolute top-4 right-4 p-2 rounded-full text-gray-400 hover:bg-gray-100 dark:hover:bg-white/5"><Settings className="w-5 h-5" /></button>}

              <div className="relative w-28 h-28 sm:w-32 sm:h-32 mx-auto mb-6 group">
                 <div className="w-full h-full rounded-full overflow-hidden ring-4 ring-emerald-100 dark:ring-emerald-900/50 shadow-xl relative bg-white dark:bg-gray-700">
                 ...
                 {!isReadOnly && <label className="absolute bottom-0 right-0 bg-white dark:bg-gray-700 p-2 rounded-full shadow-lg cursor-pointer hover:scale-110 transition-transform text-emerald-600 dark:text-emerald-400 z-30 border border-gray-100 dark:border-gray-600"><Camera className="w-5 h-5" /><input type="file" className="hidden" accept="image/*" onChange={handleAvatarChange} disabled={isUploading} /></label>}
              </div>
              ...
              <div className="flex items-center justify-center gap-1.5 mb-3">
                  <span className="bg-gray-100 dark:bg-gray-800 dark:text-gray-400 text-gray-500 text-xs px-2 py-0.5 rounded-md font-mono flex items-center gap-1 select-all"><Hash className="w-3 h-3" />{user.numericId || '000000'}</span>
              </div>
              ...
              {(!isReadOnly || (isReadOnly && showEmail)) && (
                <div className="flex items-center justify-center gap-2 text-sm text-gray-500 dark:text-gray-400 mb-4">
                    ...
                    {(!showEmail && !isReadOnly) && <span className="flex items-center gap-1 text-xs text-gray-400 bg-gray-100 dark:bg-gray-800 px-1.5 py-0.5 rounded"><EyeOff className="w-3 h-3" /> Скрыт</span>}
                </div>
              )}

              {dietaryPrefs.length > 0 && <div className="flex flex-wrap justify-center gap-2 mb-6">{dietaryPrefs.map(pref => <span key={pref} className="px-2 py-1 rounded-md bg-emerald-50 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400 text-xs font-bold border border-emerald-100 dark:border-emerald-800">{DIETARY_LABELS[pref] || pref}</span>)}</div>}
              ...
              <div className="text-left space-y-4 mb-6">
                 ...
                 <div className="bg-white/40 dark:bg-black/20 rounded-xl p-4 border border-white/20 dark:border-white/5 relative">
                    <div className="flex items-center justify-between mb-2"><span className="text-xs font-bold text-emerald-600 dark:text-emerald-400 uppercase">О себе</span>{!isEditing && !isReadOnly && <button onClick={() => setIsEditing(true)} className="text-gray-400 hover:text-emerald-500"><Edit2 className="w-3 h-3" /></button>}</div>
        ...
        {( (!isReadOnly) || (isReadOnly && showFavorites) ) ? (
            <div className={`md:col-span-2 ${isReadOnly ? 'max-w-4xl mx-auto w-full mt-8' : ''}`}>
                <div className="flex items-center gap-3 mb-6 sm:mb-8">
                    <div className="p-2 rounded-lg bg-red-50 dark:bg-red-900/20 text-red-500"><Heart className="w-6 h-6 fill-current" /></div>
                    ...
                    <span className="px-3 py-1 rounded-full bg-gray-100 dark:bg-gray-800 text-xs sm:text-sm font-bold text-gray-500 dark:text-gray-400">{favoriteRecipesList.length}</span>
                </div>
                {isLoadingFavs ? <div className="flex justify-center py-20"><Loader2 className="w-8 h-8 animate-spin text-emerald-500" /></div> : favoriteRecipesList.length === 0 ? <div className="glass-panel rounded-3xl p-12 text-center border-dashed border-2 border-gray-300 dark:border-gray-700 dark:bg-gray-800/30"><p className="text-gray-500 dark:text-gray-400 mb-4">{isReadOnly ? "Список избранного пуст." : "У вас пока нет избранных рецептов."}</p></div> : <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">{favoriteRecipesList.map(recipe => <RecipeCard key={recipe.id} recipe={recipe} onClick={onRecipeClick} onTagClick={onTagClick} />)}</div>}
            </div>
        ) : ( isReadOnly && !showFavorites && <div className="md:col-span-1 max-w-2xl mx-auto w-full mt-8 flex flex-col items-center justify-center p-10 text-center"><div className="p-4 bg-gray-100 dark:bg-gray-800 rounded-full mb-4"><Lock className="w-8 h-8 text-gray-400" /></div><h3 className="text-xl font-bold text-gray-700 dark:text-gray-300">Избранное скрыто</h3></div> )}
      </div>
